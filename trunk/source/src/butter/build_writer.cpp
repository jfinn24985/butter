...
void build_writer::find_hdr_link(const ::UmlItem & a_item, QString & a_hdr, QString & a_link, QString & a_flag, QString a_section_label, bool a_not_doc)
{
// Process any data from properties.
QString value_;
if (a_item.property_value (item_traits::butter_flags_name, value_))
{
  merge_string_list (a_flag, value_);
}
if (a_item.property_value (item_traits::butter_ldflags_name, value_))
{
  merge_string_list (a_link, value_);
}
if (a_item.property_value (item_traits::butter_include_name, value_))
{
...
std::auto_ptr< compound_artifact > build_writer::get_artifact(location & a_loc, QString a_name)
...
if (a_loc.packages ().count () > 0)
{
  ...
  if (! docs_.isEmpty ())
  {
    MSC_NO_RESET (Result, std::auto_ptr< compound_artifact >)(new compound_artifact (dynamic_cast< UmlArtifact& >(*docs_.at (0))));
...
if (NULL == Result.get ())
{
  ...
  MSC_NO_RESET (Result, std::auto_ptr< compound_artifact >)(new compound_artifact (*a_loc.create_uml_document (a_name)));
}
return Result;

}

void build_writer::merge_string_list(QString & a_list, QString a_addition)
{
if (a_list.isEmpty ())
{
  a_list = a_addition.stripWhiteSpace ();
}
else
{
...
QString build_writer::section(QString a_section, QString a_desc)
{
...
QString build_writer::to_target_NAME(const ::UmlArtifact & a_target)
{
