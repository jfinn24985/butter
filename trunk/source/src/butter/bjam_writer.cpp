 ...
 */
const QString bjam_writer::build_file_name("Jamfile");

/**
 * The default leaf filename for the current style
 */
const QString bjam_writer::build_file_sysname("Jamroot");

/**
 * Comment line prefix
 */
const QString bjam_writer::comment_string("#");

const QString bjam_writer::end_phrase("##END:");

const QString bjam_writer::start_phrase("##START:");

/**
 * The label for description 'sections' and the value of 
 * the style for this buildfile type.
 */
const QString bjam_writer::section_name("boost");

/**
 * The name of the project rules file.
 */
const QString bjam_writer::rules_name("local.jam");
 ...
 * using this string.
 */
const char * bjam_writer::default_rules[] = { "#\n"
, "# local.jam\n"
, "#\n"
...
void bjam_writer::assoc_library(const ::UmlArtifact & a_target, QTextStream & a_os, QString & a_includes, QString & a_ldflags, QString & a_cflags) {
QString project_;
...
void bjam_writer::assoc_source(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_filename, QString a_basename, QString a_src_inc, QString a_src_flags, bool a_isdoc) {
QString individual_;
if (! this->is_other_ && (! a_src_inc.isEmpty () && ! a_src_flags.isEmpty ()))
{
  ...
  BUTTER_ALWAYS(a_target.property_value (item_traits::butter_compiler_name, compiler_) || ! a_isdoc
...
std::auto_ptr< build_writer > bjam_writer::create()

{
  std::auto_ptr< build_writer > Result (new bjam_writer);
  return Result;
}

void bjam_writer::descendent_link(compound_artifact & a_art, compound_artifact & a_sys, const location & a_loc) {
...
void bjam_writer::end_target(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_include, QString a_ldflags, QString a_cflags, QString a_compiler, build_writer::target_type a_type) {
///////////////////////
...
case static_library:
  {
    ...
    QString headers_;
    if (a_target.property_value (item_traits::butter_include_name, headers_))
    {
...
if (! this->individual_obj_.isEmpty ())
{
  ...
  QString flags_;
  if (a_target.property_value (item_traits::butter_flags_name, flags_))
  {
...
void bjam_writer::external_target(const location & a_current, const ::UmlArtifact & a_target, compound_artifact & a_sys) {
// Get project name, if present
QString value_;
a_target.property_value (item_traits::butter_project_name, value_);
...
{
  QString tmp_;
  if (a_target.property_value (item_traits::butter_buildfile_name, tmp_))
  {
...
void bjam_writer::initialise(location & a_base, const ::UmlItem & a_project, compound_artifact & a_sys) {
...
void bjam_writer::install_target(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_loc_var, build_writer::install_type a_type, bool a_isdoc) 
{
...
void bjam_writer::start_target(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_build_file, QString a_compiler, build_writer::target_type a_type) {
////////////////////////
// Reset variables
...
{
  if (a_target.property_value (item_traits::butter_other_name, this->other_target_type_))
  {
