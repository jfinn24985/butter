 ...
 */
const QString gmake_writer::build_file_name("makefile");

/**
 * The default leaf filename for the current style
 */
const QString gmake_writer::build_file_sysname("makefile");

/**
 * Comment line prefix
 */
const QString gmake_writer::comment_string("#");

const QString gmake_writer::end_phrase("##END:");

const QString gmake_writer::start_phrase("##START:");

/**
 * The label for description 'sections' and the value of 
 * the style for this buildfile type.
 */
const QString gmake_writer::section_name("make");
 ...
 * using this string.
 */
const char * gmake_writer::default_rules_sys[] = { "#\n"
, "# M_sys.mak\n"
, "#\n"
, "##################################\n"
 ...
 * 
 */
const char * gmake_writer::default_rules_cl[] = { "##########################################\n"
 ...
 * using this string.
 */
const char * gmake_writer::default_rules_gcc[] = { "####################################\n"
 ...
 * 
 */
const char * gmake_writer::default_rules_unix[] = { " # Definition of for loop in local shell.\n"
, "define doit\n"
 ...
 * using this string.
 */
const char * gmake_writer::default_rules_winnt[] = { "# Definition of for loop in local shell.\n\n"
, "define doit\n"
, "$(if $(2), set errorlevel=& set STATUS=0 & for %%W in ( $(2) ) do ( $(MAKE) -C %%W $(1) & if errorlevel ( set STATUS=%errorlevel% ) ) & exit %STATUS%)\n"
, "endef\n"
, "\n"
, "ifndef M_WINNT_MAK\n"
, "M_WINNT_MAK:=1\n"
...
, "endif\n\n"
, "define do_install\n"
, "install:: $(1) ; $$(MKDIR) $$($(2)DIR) && $$(COPY) $(1) $$($(2)DIR)/$(1)\n"
, "endef\n\n"
, "###############\n"
, "## Editor sugar\n"
, "###############\n"
 ...
 */
const char ** gmake_writer::default_rules_list[] = { default_rules_sys, default_rules_cl, default_rules_gcc, default_rules_unix, default_rules_winnt, 0 };
 ...
 */
const char ** gmake_writer::default_rules = combine(gmake_writer::default_rules_list);
 ...
 */
const QString gmake_writer::rules_name("M_sys.mak M_cl.mak M_gcc.mak M_unix.mak M_Windows_NT.mak");

void gmake_writer::assoc_library(const ::UmlArtifact & a_target, QTextStream & a_os, QString & a_includes, QString & a_ldflags, QString & a_cflags) {
////////////////////////////////////
...
void gmake_writer::assoc_source(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_filename, QString a_basename, QString a_src_inc, QString a_src_flags, bool a_isdoc) {
////////////////////////////////////
// Define the associations of target
//
...
std::auto_ptr< build_writer > gmake_writer::create()

{
  std::auto_ptr< build_writer > Result (new gmake_writer);
  return Result;
}

void gmake_writer::descendent_link(compound_artifact & a_art, compound_artifact & a_sys, const location & a_loc) 
{
...
void gmake_writer::end_target(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_include, QString a_ldflags, QString a_cflags, QString a_compiler, build_writer::target_type a_type) {
////////////////////////////////
...
gmake_writer::gmake_writer()
: lib_set_ ()
, qualified_target_name_ ()
{}

void gmake_writer::initialise(location & a_base, const ::UmlItem & a_project, compound_artifact & a_sys) 
{
...
QString init_text_;
{
  ...
  QString inc_;
  a_project.property_value (item_traits::butter_include_name, inc_);
  ...
  // Get list of flags
  QString flags_;
  a_project.property_search (item_traits::butter_flags_name, flags_);
  ...
  // Get linker flags
  QString linkflag_;
  if (a_project.property_search (item_traits::butter_ldflags_name, linkflag_))
  {
...
void gmake_writer::install_target(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_loc_var, build_writer::install_type a_type, bool a_isdoc) 
{
...
void gmake_writer::process_flags(QString a_inc_list, QString a_flag_list, QString & a_cppflags, QString & a_cflags) 
{
...
QString gmake_writer::process_hdrs(QString a_inc_list)
{
QString Result;
...
bool gmake_writer::requirements(const ::UmlItem & a_item, QString & a_reqs)
{
bool Result (false);
QString l_tmp;
QString l_section;
QTextOStream l_os (&l_section);
if (a_item.property_value (item_traits::butter_include_name, l_tmp))
{
  Result = true;
  l_os << "USERHDRS := $(USERHDRS) " << process_hdrs (l_tmp) << "\n";
}
if (a_item.property_search (item_traits::butter_ldflags_name, l_tmp))
{
  Result = true;
  l_os << "USERLDFLAGS := $(USERLDFLAGS) " << l_tmp << "\n";
}
if (a_item.property_search (item_traits::butter_flags_name, l_tmp))
{
  Result = true;
...
void gmake_writer::start_target(const ::UmlArtifact & a_target, QTextStream & a_os, QString a_build_file, QString a_compiler, build_writer::target_type a_type) {
/////////////////
